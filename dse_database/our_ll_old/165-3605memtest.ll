; ModuleID = './code/165-3605memtest.c'
source_filename = "./code/165-3605memtest.c"
target datalayout = "e-m:o-i64:64-i128:128-n32:64-S128-Fn32"
target triple = "arm64-apple-macosx14.0.0"

@.str = private unnamed_addr constant [11 x i8] c"First ... \00", align 1
@.str.1 = private unnamed_addr constant [6 x i8] c"done\0A\00", align 1
@.str.2 = private unnamed_addr constant [12 x i8] c"Second ... \00", align 1

; Function Attrs: noinline nounwind optnone ssp uwtable(sync)
define ptr @myalloc_data_(i64 noundef %size) #0 {
entry:
  %size.addr = alloca i64, align 8
  %i = alloca i32, align 4
  %tmp = alloca ptr, align 8
  %buffer = alloca ptr, align 8
  %elem_size = alloca i32, align 4
  store i64 %size, ptr %size.addr, align 8
  store i32 0, ptr %i, align 4
  store ptr null, ptr %tmp, align 8
  %0 = load i64, ptr %size.addr, align 8
  %call = call ptr @calloc(i64 noundef %0, i64 noundef 8) #4
  store ptr %call, ptr %buffer, align 8
  store i32 0, ptr %i, align 4
  %1 = load ptr, ptr %buffer, align 8
  store ptr %1, ptr %tmp, align 8
  br label %for.cond

for.cond:                                         ; preds = %for.inc, %entry
  %2 = load i32, ptr %i, align 4
  %conv = sext i32 %2 to i64
  %3 = load i64, ptr %size.addr, align 8
  %cmp = icmp ult i64 %conv, %3
  br i1 %cmp, label %for.body, label %for.end

for.body:                                         ; preds = %for.cond
  %call2 = call i32 @rand()
  %rem = srem i32 %call2, 10
  %add = add nsw i32 256, %rem
  store i32 %add, ptr %elem_size, align 4
  %4 = load i32, ptr %elem_size, align 4
  %conv3 = sext i32 %4 to i64
  %call4 = call ptr @malloc(i64 noundef %conv3) #5
  %5 = load ptr, ptr %tmp, align 8
  store ptr %call4, ptr %5, align 8
  br label %for.inc

for.inc:                                          ; preds = %for.body
  %6 = load i32, ptr %i, align 4
  %inc = add nsw i32 %6, 1
  store i32 %inc, ptr %i, align 4
  %7 = load ptr, ptr %tmp, align 8
  %incdec.ptr = getelementptr inbounds ptr, ptr %7, i32 1
  store ptr %incdec.ptr, ptr %tmp, align 8
  br label %for.cond, !llvm.loop !5

for.end:                                          ; preds = %for.cond
  %8 = load ptr, ptr %buffer, align 8
  ret ptr %8
}

; Function Attrs: allocsize(0,1)
declare ptr @calloc(i64 noundef, i64 noundef) #1

declare i32 @rand() #2

; Function Attrs: allocsize(0)
declare ptr @malloc(i64 noundef) #3

; Function Attrs: noinline nounwind optnone ssp uwtable(sync)
define void @myfree_data_(ptr noundef %buffer, i64 noundef %size) #0 {
entry:
  %buffer.addr = alloca ptr, align 8
  %size.addr = alloca i64, align 8
  %i = alloca i32, align 4
  %tmp = alloca ptr, align 8
  store ptr %buffer, ptr %buffer.addr, align 8
  store i64 %size, ptr %size.addr, align 8
  store i32 0, ptr %i, align 4
  store ptr null, ptr %tmp, align 8
  store i32 0, ptr %i, align 4
  %0 = load ptr, ptr %buffer.addr, align 8
  store ptr %0, ptr %tmp, align 8
  br label %for.cond

for.cond:                                         ; preds = %for.inc, %entry
  %1 = load i32, ptr %i, align 4
  %conv = sext i32 %1 to i64
  %2 = load i64, ptr %size.addr, align 8
  %cmp = icmp ult i64 %conv, %2
  br i1 %cmp, label %for.body, label %for.end

for.body:                                         ; preds = %for.cond
  %3 = load ptr, ptr %tmp, align 8
  %4 = load ptr, ptr %3, align 8
  call void @free(ptr noundef %4)
  br label %for.inc

for.inc:                                          ; preds = %for.body
  %5 = load i32, ptr %i, align 4
  %inc = add nsw i32 %5, 1
  store i32 %inc, ptr %i, align 4
  %6 = load ptr, ptr %tmp, align 8
  %incdec.ptr = getelementptr inbounds ptr, ptr %6, i32 1
  store ptr %incdec.ptr, ptr %tmp, align 8
  br label %for.cond, !llvm.loop !7

for.end:                                          ; preds = %for.cond
  %7 = load ptr, ptr %buffer.addr, align 8
  call void @free(ptr noundef %7)
  ret void
}

declare void @free(ptr noundef) #2

; Function Attrs: noinline nounwind optnone ssp uwtable(sync)
define i32 @main() #0 {
entry:
  %retval = alloca i32, align 4
  %buffer = alloca ptr, align 8
  %size1 = alloca i64, align 8
  %size2 = alloca i64, align 8
  store i32 0, ptr %retval, align 4
  store i64 3145728, ptr %size1, align 8
  store i64 1048576, ptr %size2, align 8
  %call = call i32 (ptr, ...) @printf(ptr noundef @.str)
  %0 = load i64, ptr %size1, align 8
  %call1 = call ptr @myalloc_data_(i64 noundef %0)
  store ptr %call1, ptr %buffer, align 8
  %call2 = call i32 (ptr, ...) @printf(ptr noundef @.str.1)
  %call3 = call i32 @"\01_sleep"(i32 noundef 10)
  %1 = load ptr, ptr %buffer, align 8
  %2 = load i64, ptr %size1, align 8
  call void @myfree_data_(ptr noundef %1, i64 noundef %2)
  %call4 = call i32 (ptr, ...) @printf(ptr noundef @.str.2)
  %3 = load i64, ptr %size2, align 8
  %call5 = call ptr @myalloc_data_(i64 noundef %3)
  store ptr %call5, ptr %buffer, align 8
  %call6 = call i32 (ptr, ...) @printf(ptr noundef @.str.1)
  %call7 = call i32 @"\01_sleep"(i32 noundef 10)
  %4 = load ptr, ptr %buffer, align 8
  %5 = load i64, ptr %size2, align 8
  call void @myfree_data_(ptr noundef %4, i64 noundef %5)
  ret i32 0
}

declare i32 @printf(ptr noundef, ...) #2

declare i32 @"\01_sleep"(i32 noundef) #2

attributes #0 = { noinline nounwind optnone ssp uwtable(sync) "frame-pointer"="non-leaf" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+altnzcv,+ccdp,+ccidx,+complxnum,+crc,+dit,+dotprod,+flagm,+fp-armv8,+fp16fml,+fptoint,+fullfp16,+jsconv,+lse,+neon,+pauth,+perfmon,+predres,+ras,+rcpc,+rdm,+sb,+sha2,+sha3,+specrestrict,+ssbs,+v8.1a,+v8.2a,+v8.3a,+v8.4a,+v8a,+zcm,+zcz" }
attributes #1 = { allocsize(0,1) "frame-pointer"="non-leaf" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+altnzcv,+ccdp,+ccidx,+complxnum,+crc,+dit,+dotprod,+flagm,+fp-armv8,+fp16fml,+fptoint,+fullfp16,+jsconv,+lse,+neon,+pauth,+perfmon,+predres,+ras,+rcpc,+rdm,+sb,+sha2,+sha3,+specrestrict,+ssbs,+v8.1a,+v8.2a,+v8.3a,+v8.4a,+v8a,+zcm,+zcz" }
attributes #2 = { "frame-pointer"="non-leaf" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+altnzcv,+ccdp,+ccidx,+complxnum,+crc,+dit,+dotprod,+flagm,+fp-armv8,+fp16fml,+fptoint,+fullfp16,+jsconv,+lse,+neon,+pauth,+perfmon,+predres,+ras,+rcpc,+rdm,+sb,+sha2,+sha3,+specrestrict,+ssbs,+v8.1a,+v8.2a,+v8.3a,+v8.4a,+v8a,+zcm,+zcz" }
attributes #3 = { allocsize(0) "frame-pointer"="non-leaf" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+altnzcv,+ccdp,+ccidx,+complxnum,+crc,+dit,+dotprod,+flagm,+fp-armv8,+fp16fml,+fptoint,+fullfp16,+jsconv,+lse,+neon,+pauth,+perfmon,+predres,+ras,+rcpc,+rdm,+sb,+sha2,+sha3,+specrestrict,+ssbs,+v8.1a,+v8.2a,+v8.3a,+v8.4a,+v8a,+zcm,+zcz" }
attributes #4 = { allocsize(0,1) }
attributes #5 = { allocsize(0) }

!llvm.module.flags = !{!0, !1, !2, !3}
!llvm.ident = !{!4}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"uwtable", i32 1}
!3 = !{i32 7, !"frame-pointer", i32 1}
!4 = !{!"Homebrew clang version 19.1.4"}
!5 = distinct !{!5, !6}
!6 = !{!"llvm.loop.mustprogress"}
!7 = distinct !{!7, !6}
