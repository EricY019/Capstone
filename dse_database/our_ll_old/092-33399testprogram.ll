; ModuleID = './code/092-33399testprogram.c'
source_filename = "./code/092-33399testprogram.c"
target datalayout = "e-m:o-i64:64-i128:128-n32:64-S128-Fn32"
target triple = "arm64-apple-macosx14.0.0"

@.str = private unnamed_addr constant [11 x i8] c"Wynik: %d\0A\00", align 1

; Function Attrs: noinline nounwind optnone ssp uwtable(sync)
define i32 @main() #0 {
entry:
  %retval = alloca i32, align 4
  %size = alloca i32, align 4
  %jump = alloca i32, align 4
  %saved_stack = alloca ptr, align 8
  %__vla_expr0 = alloca i64, align 8
  %suma = alloca i64, align 8
  %i = alloca i32, align 4
  %j = alloca i32, align 4
  %k = alloca i32, align 4
  %i7 = alloca i32, align 4
  store i32 0, ptr %retval, align 4
  store i32 1024, ptr %size, align 4
  store i32 256, ptr %jump, align 4
  %0 = load i32, ptr %size, align 4
  %1 = zext i32 %0 to i64
  %2 = call ptr @llvm.stacksave.p0()
  store ptr %2, ptr %saved_stack, align 8
  %vla = alloca i64, i64 %1, align 8
  store i64 %1, ptr %__vla_expr0, align 8
  store i32 0, ptr %i, align 4
  br label %for.cond

for.cond:                                         ; preds = %for.inc, %entry
  %3 = load i32, ptr %i, align 4
  %4 = load i32, ptr %size, align 4
  %cmp = icmp slt i32 %3, %4
  br i1 %cmp, label %for.body, label %for.end

for.body:                                         ; preds = %for.cond
  %5 = load i32, ptr %i, align 4
  %idxprom = sext i32 %5 to i64
  %arrayidx = getelementptr inbounds i64, ptr %vla, i64 %idxprom
  store i64 0, ptr %arrayidx, align 8
  br label %for.inc

for.inc:                                          ; preds = %for.body
  %6 = load i32, ptr %i, align 4
  %inc = add nsw i32 %6, 1
  store i32 %inc, ptr %i, align 4
  br label %for.cond, !llvm.loop !5

for.end:                                          ; preds = %for.cond
  store i32 0, ptr %j, align 4
  br label %for.cond1

for.cond1:                                        ; preds = %for.inc24, %for.end
  %7 = load i32, ptr %j, align 4
  %cmp2 = icmp slt i32 %7, 1000
  br i1 %cmp2, label %for.body3, label %for.end26

for.body3:                                        ; preds = %for.cond1
  store i32 0, ptr %k, align 4
  br label %for.cond4

for.cond4:                                        ; preds = %for.inc21, %for.body3
  %8 = load i32, ptr %k, align 4
  %9 = load i32, ptr %jump, align 4
  %cmp5 = icmp slt i32 %8, %9
  br i1 %cmp5, label %for.body6, label %for.end23

for.body6:                                        ; preds = %for.cond4
  %10 = load i32, ptr %k, align 4
  store i32 %10, ptr %i7, align 4
  br label %for.cond8

for.cond8:                                        ; preds = %for.inc18, %for.body6
  %11 = load i32, ptr %i7, align 4
  %12 = load i32, ptr %size, align 4
  %cmp9 = icmp slt i32 %11, %12
  br i1 %cmp9, label %for.body10, label %for.end20

for.body10:                                       ; preds = %for.cond8
  %13 = load i32, ptr %i7, align 4
  %idxprom11 = sext i32 %13 to i64
  %arrayidx12 = getelementptr inbounds i64, ptr %vla, i64 %idxprom11
  %14 = load i64, ptr %arrayidx12, align 8
  %15 = load i32, ptr %i7, align 4
  %conv = sext i32 %15 to i64
  %add = add nsw i64 %14, %conv
  %16 = load i32, ptr %i7, align 4
  %idxprom13 = sext i32 %16 to i64
  %arrayidx14 = getelementptr inbounds i64, ptr %vla, i64 %idxprom13
  store i64 %add, ptr %arrayidx14, align 8
  %17 = load i32, ptr %i7, align 4
  %idxprom15 = sext i32 %17 to i64
  %arrayidx16 = getelementptr inbounds i64, ptr %vla, i64 %idxprom15
  %18 = load i64, ptr %arrayidx16, align 8
  %19 = load i64, ptr %suma, align 8
  %add17 = add nsw i64 %19, %18
  store i64 %add17, ptr %suma, align 8
  br label %for.inc18

for.inc18:                                        ; preds = %for.body10
  %20 = load i32, ptr %jump, align 4
  %21 = load i32, ptr %i7, align 4
  %add19 = add nsw i32 %21, %20
  store i32 %add19, ptr %i7, align 4
  br label %for.cond8, !llvm.loop !7

for.end20:                                        ; preds = %for.cond8
  br label %for.inc21

for.inc21:                                        ; preds = %for.end20
  %22 = load i32, ptr %k, align 4
  %inc22 = add nsw i32 %22, 1
  store i32 %inc22, ptr %k, align 4
  br label %for.cond4, !llvm.loop !8

for.end23:                                        ; preds = %for.cond4
  br label %for.inc24

for.inc24:                                        ; preds = %for.end23
  %23 = load i32, ptr %j, align 4
  %inc25 = add nsw i32 %23, 1
  store i32 %inc25, ptr %j, align 4
  br label %for.cond1, !llvm.loop !9

for.end26:                                        ; preds = %for.cond1
  %24 = load i64, ptr %suma, align 8
  %call = call i32 (ptr, ...) @printf(ptr noundef @.str, i64 noundef %24)
  %25 = load ptr, ptr %saved_stack, align 8
  call void @llvm.stackrestore.p0(ptr %25)
  %26 = load i32, ptr %retval, align 4
  ret i32 %26
}

; Function Attrs: nocallback nofree nosync nounwind willreturn
declare ptr @llvm.stacksave.p0() #1

declare i32 @printf(ptr noundef, ...) #2

; Function Attrs: nocallback nofree nosync nounwind willreturn
declare void @llvm.stackrestore.p0(ptr) #1

attributes #0 = { noinline nounwind optnone ssp uwtable(sync) "frame-pointer"="non-leaf" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+altnzcv,+ccdp,+ccidx,+complxnum,+crc,+dit,+dotprod,+flagm,+fp-armv8,+fp16fml,+fptoint,+fullfp16,+jsconv,+lse,+neon,+pauth,+perfmon,+predres,+ras,+rcpc,+rdm,+sb,+sha2,+sha3,+specrestrict,+ssbs,+v8.1a,+v8.2a,+v8.3a,+v8.4a,+v8a,+zcm,+zcz" }
attributes #1 = { nocallback nofree nosync nounwind willreturn }
attributes #2 = { "frame-pointer"="non-leaf" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+altnzcv,+ccdp,+ccidx,+complxnum,+crc,+dit,+dotprod,+flagm,+fp-armv8,+fp16fml,+fptoint,+fullfp16,+jsconv,+lse,+neon,+pauth,+perfmon,+predres,+ras,+rcpc,+rdm,+sb,+sha2,+sha3,+specrestrict,+ssbs,+v8.1a,+v8.2a,+v8.3a,+v8.4a,+v8a,+zcm,+zcz" }

!llvm.module.flags = !{!0, !1, !2, !3}
!llvm.ident = !{!4}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"uwtable", i32 1}
!3 = !{i32 7, !"frame-pointer", i32 1}
!4 = !{!"Homebrew clang version 19.1.4"}
!5 = distinct !{!5, !6}
!6 = !{!"llvm.loop.mustprogress"}
!7 = distinct !{!7, !6}
!8 = distinct !{!8, !6}
!9 = distinct !{!9, !6}
