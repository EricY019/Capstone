; ModuleID = './code/292-13873sorted_insert.c'
source_filename = "./code/292-13873sorted_insert.c"
target datalayout = "e-m:o-i64:64-i128:128-n32:64-S128-Fn32"
target triple = "arm64-apple-macosx14.0.0"

@.str = private unnamed_addr constant [20 x i8] c"\0ABefore Insertion: \00", align 1
@.str.1 = private unnamed_addr constant [4 x i8] c"%d \00", align 1
@.str.2 = private unnamed_addr constant [19 x i8] c"\0AAfter Insertion: \00", align 1

; Function Attrs: noinline nounwind optnone ssp uwtable(sync)
define i32 @insertSorted(ptr noundef %array, i32 noundef %n, i32 noundef %key, i32 noundef %capacity) #0 {
entry:
  %retval = alloca i32, align 4
  %array.addr = alloca ptr, align 8
  %n.addr = alloca i32, align 4
  %key.addr = alloca i32, align 4
  %capacity.addr = alloca i32, align 4
  %i = alloca i32, align 4
  store ptr %array, ptr %array.addr, align 8
  store i32 %n, ptr %n.addr, align 4
  store i32 %key, ptr %key.addr, align 4
  store i32 %capacity, ptr %capacity.addr, align 4
  %0 = load i32, ptr %n.addr, align 4
  %1 = load i32, ptr %capacity.addr, align 4
  %cmp = icmp sge i32 %0, %1
  br i1 %cmp, label %if.then, label %if.end

if.then:                                          ; preds = %entry
  %2 = load i32, ptr %n.addr, align 4
  store i32 %2, ptr %retval, align 4
  br label %return

if.end:                                           ; preds = %entry
  %3 = load i32, ptr %n.addr, align 4
  %sub = sub nsw i32 %3, 1
  store i32 %sub, ptr %i, align 4
  br label %for.cond

for.cond:                                         ; preds = %for.inc, %if.end
  %4 = load ptr, ptr %array.addr, align 8
  %5 = load i32, ptr %i, align 4
  %idxprom = sext i32 %5 to i64
  %arrayidx = getelementptr inbounds i32, ptr %4, i64 %idxprom
  %6 = load i32, ptr %arrayidx, align 4
  %7 = load i32, ptr %key.addr, align 4
  %cmp1 = icmp sgt i32 %6, %7
  br i1 %cmp1, label %land.rhs, label %land.end

land.rhs:                                         ; preds = %for.cond
  %8 = load i32, ptr %i, align 4
  %cmp2 = icmp sge i32 %8, 0
  br label %land.end

land.end:                                         ; preds = %land.rhs, %for.cond
  %9 = phi i1 [ false, %for.cond ], [ %cmp2, %land.rhs ]
  br i1 %9, label %for.body, label %for.end

for.body:                                         ; preds = %land.end
  %10 = load ptr, ptr %array.addr, align 8
  %11 = load i32, ptr %i, align 4
  %idxprom3 = sext i32 %11 to i64
  %arrayidx4 = getelementptr inbounds i32, ptr %10, i64 %idxprom3
  %12 = load i32, ptr %arrayidx4, align 4
  %13 = load ptr, ptr %array.addr, align 8
  %14 = load i32, ptr %i, align 4
  %add = add nsw i32 %14, 1
  %idxprom5 = sext i32 %add to i64
  %arrayidx6 = getelementptr inbounds i32, ptr %13, i64 %idxprom5
  store i32 %12, ptr %arrayidx6, align 4
  br label %for.inc

for.inc:                                          ; preds = %for.body
  %15 = load i32, ptr %i, align 4
  %dec = add nsw i32 %15, -1
  store i32 %dec, ptr %i, align 4
  br label %for.cond, !llvm.loop !5

for.end:                                          ; preds = %land.end
  %16 = load i32, ptr %key.addr, align 4
  %17 = load ptr, ptr %array.addr, align 8
  %18 = load i32, ptr %i, align 4
  %add7 = add nsw i32 %18, 1
  %idxprom8 = sext i32 %add7 to i64
  %arrayidx9 = getelementptr inbounds i32, ptr %17, i64 %idxprom8
  store i32 %16, ptr %arrayidx9, align 4
  %19 = load i32, ptr %n.addr, align 4
  %add10 = add nsw i32 %19, 1
  store i32 %add10, ptr %retval, align 4
  br label %return

return:                                           ; preds = %for.end, %if.then
  %20 = load i32, ptr %retval, align 4
  ret i32 %20
}

; Function Attrs: noinline nounwind optnone ssp uwtable(sync)
define i32 @main() #0 {
entry:
  %retval = alloca i32, align 4
  %array = alloca [20 x i32], align 4
  %capacity = alloca i32, align 4
  %n = alloca i32, align 4
  %i = alloca i32, align 4
  %key = alloca i32, align 4
  store i32 0, ptr %retval, align 4
  call void @llvm.memset.p0.i64(ptr align 4 %array, i8 0, i64 80, i1 false)
  %0 = getelementptr inbounds <{ i32, i32, i32, i32, i32, i32, [14 x i32] }>, ptr %array, i32 0, i32 0
  store i32 12, ptr %0, align 4
  %1 = getelementptr inbounds <{ i32, i32, i32, i32, i32, i32, [14 x i32] }>, ptr %array, i32 0, i32 1
  store i32 16, ptr %1, align 4
  %2 = getelementptr inbounds <{ i32, i32, i32, i32, i32, i32, [14 x i32] }>, ptr %array, i32 0, i32 2
  store i32 20, ptr %2, align 4
  %3 = getelementptr inbounds <{ i32, i32, i32, i32, i32, i32, [14 x i32] }>, ptr %array, i32 0, i32 3
  store i32 40, ptr %3, align 4
  %4 = getelementptr inbounds <{ i32, i32, i32, i32, i32, i32, [14 x i32] }>, ptr %array, i32 0, i32 4
  store i32 50, ptr %4, align 4
  %5 = getelementptr inbounds <{ i32, i32, i32, i32, i32, i32, [14 x i32] }>, ptr %array, i32 0, i32 5
  store i32 70, ptr %5, align 4
  store i32 20, ptr %capacity, align 4
  store i32 6, ptr %n, align 4
  store i32 26, ptr %key, align 4
  %call = call i32 (ptr, ...) @printf(ptr noundef @.str)
  store i32 0, ptr %i, align 4
  br label %for.cond

for.cond:                                         ; preds = %for.inc, %entry
  %6 = load i32, ptr %i, align 4
  %7 = load i32, ptr %n, align 4
  %cmp = icmp slt i32 %6, %7
  br i1 %cmp, label %for.body, label %for.end

for.body:                                         ; preds = %for.cond
  %8 = load i32, ptr %i, align 4
  %idxprom = sext i32 %8 to i64
  %arrayidx = getelementptr inbounds [20 x i32], ptr %array, i64 0, i64 %idxprom
  %9 = load i32, ptr %arrayidx, align 4
  %call1 = call i32 (ptr, ...) @printf(ptr noundef @.str.1, i32 noundef %9)
  br label %for.inc

for.inc:                                          ; preds = %for.body
  %10 = load i32, ptr %i, align 4
  %inc = add nsw i32 %10, 1
  store i32 %inc, ptr %i, align 4
  br label %for.cond, !llvm.loop !7

for.end:                                          ; preds = %for.cond
  %arraydecay = getelementptr inbounds [20 x i32], ptr %array, i64 0, i64 0
  %11 = load i32, ptr %n, align 4
  %12 = load i32, ptr %key, align 4
  %13 = load i32, ptr %capacity, align 4
  %call2 = call i32 @insertSorted(ptr noundef %arraydecay, i32 noundef %11, i32 noundef %12, i32 noundef %13)
  store i32 %call2, ptr %n, align 4
  %call3 = call i32 (ptr, ...) @printf(ptr noundef @.str.2)
  store i32 0, ptr %i, align 4
  br label %for.cond4

for.cond4:                                        ; preds = %for.inc10, %for.end
  %14 = load i32, ptr %i, align 4
  %15 = load i32, ptr %n, align 4
  %cmp5 = icmp slt i32 %14, %15
  br i1 %cmp5, label %for.body6, label %for.end12

for.body6:                                        ; preds = %for.cond4
  %16 = load i32, ptr %i, align 4
  %idxprom7 = sext i32 %16 to i64
  %arrayidx8 = getelementptr inbounds [20 x i32], ptr %array, i64 0, i64 %idxprom7
  %17 = load i32, ptr %arrayidx8, align 4
  %call9 = call i32 (ptr, ...) @printf(ptr noundef @.str.1, i32 noundef %17)
  br label %for.inc10

for.inc10:                                        ; preds = %for.body6
  %18 = load i32, ptr %i, align 4
  %inc11 = add nsw i32 %18, 1
  store i32 %inc11, ptr %i, align 4
  br label %for.cond4, !llvm.loop !8

for.end12:                                        ; preds = %for.cond4
  ret i32 0
}

; Function Attrs: nocallback nofree nounwind willreturn memory(argmem: write)
declare void @llvm.memset.p0.i64(ptr nocapture writeonly, i8, i64, i1 immarg) #1

declare i32 @printf(ptr noundef, ...) #2

attributes #0 = { noinline nounwind optnone ssp uwtable(sync) "frame-pointer"="non-leaf" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+altnzcv,+ccdp,+ccidx,+complxnum,+crc,+dit,+dotprod,+flagm,+fp-armv8,+fp16fml,+fptoint,+fullfp16,+jsconv,+lse,+neon,+pauth,+perfmon,+predres,+ras,+rcpc,+rdm,+sb,+sha2,+sha3,+specrestrict,+ssbs,+v8.1a,+v8.2a,+v8.3a,+v8.4a,+v8a,+zcm,+zcz" }
attributes #1 = { nocallback nofree nounwind willreturn memory(argmem: write) }
attributes #2 = { "frame-pointer"="non-leaf" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+altnzcv,+ccdp,+ccidx,+complxnum,+crc,+dit,+dotprod,+flagm,+fp-armv8,+fp16fml,+fptoint,+fullfp16,+jsconv,+lse,+neon,+pauth,+perfmon,+predres,+ras,+rcpc,+rdm,+sb,+sha2,+sha3,+specrestrict,+ssbs,+v8.1a,+v8.2a,+v8.3a,+v8.4a,+v8a,+zcm,+zcz" }

!llvm.module.flags = !{!0, !1, !2, !3}
!llvm.ident = !{!4}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"uwtable", i32 1}
!3 = !{i32 7, !"frame-pointer", i32 1}
!4 = !{!"Homebrew clang version 19.1.4"}
!5 = distinct !{!5, !6}
!6 = !{!"llvm.loop.mustprogress"}
!7 = distinct !{!7, !6}
!8 = distinct !{!8, !6}
