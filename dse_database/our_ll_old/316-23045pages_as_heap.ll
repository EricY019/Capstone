; ModuleID = './code/316-23045pages_as_heap.c'
source_filename = "./code/316-23045pages_as_heap.c"
target datalayout = "e-m:o-i64:64-i128:128-n32:64-S128-Fn32"
target triple = "arm64-apple-macosx14.0.0"

@.str = private unnamed_addr constant [26 x i8] c"brk value at failure: %p\0A\00", align 1
@.str.1 = private unnamed_addr constant [15 x i8] c"brk() failed!\0A\00", align 1

; Function Attrs: noinline nounwind optnone ssp uwtable(sync)
define i32 @main() #0 {
entry:
  %retval = alloca i32, align 4
  %i = alloca i32, align 4
  %inc_dec = alloca i32, align 4
  %delta = alloca i32, align 4
  %brk_stat = alloca i64, align 8
  store i32 0, ptr %retval, align 4
  store i32 1, ptr %inc_dec, align 4
  br label %for.cond

for.cond:                                         ; preds = %for.inc13, %entry
  %0 = load i32, ptr %inc_dec, align 4
  %cmp = icmp sge i32 %0, -1
  br i1 %cmp, label %for.body, label %for.end14

for.body:                                         ; preds = %for.cond
  store i32 1, ptr %delta, align 4
  br label %for.cond1

for.cond1:                                        ; preds = %for.inc11, %for.body
  %1 = load i32, ptr %delta, align 4
  %cmp2 = icmp sle i32 %1, 400
  br i1 %cmp2, label %for.body3, label %for.end12

for.body3:                                        ; preds = %for.cond1
  store i32 0, ptr %i, align 4
  br label %for.cond4

for.cond4:                                        ; preds = %for.inc, %for.body3
  %2 = load i32, ptr %i, align 4
  %cmp5 = icmp slt i32 %2, 20000
  br i1 %cmp5, label %for.body6, label %for.end

for.body6:                                        ; preds = %for.cond4
  %call = call ptr @sbrk(i32 noundef 0)
  %3 = load i32, ptr %inc_dec, align 4
  %4 = load i32, ptr %delta, align 4
  %mul = mul nsw i32 %3, %4
  %idx.ext = sext i32 %mul to i64
  %add.ptr = getelementptr inbounds i8, ptr %call, i64 %idx.ext
  %call7 = call ptr @brk(ptr noundef %add.ptr)
  %5 = ptrtoint ptr %call7 to i64
  store i64 %5, ptr %brk_stat, align 8
  %6 = load i64, ptr %brk_stat, align 8
  %cmp8 = icmp eq i64 %6, -1
  br i1 %cmp8, label %if.then, label %if.end

if.then:                                          ; preds = %for.body6
  %call9 = call ptr @sbrk(i32 noundef 0)
  %call10 = call i32 (ptr, ...) @printf(ptr noundef @.str, ptr noundef %call9)
  call void @perror(ptr noundef @.str.1) #3
  store i32 0, ptr %retval, align 4
  br label %return

if.end:                                           ; preds = %for.body6
  br label %for.inc

for.inc:                                          ; preds = %if.end
  %7 = load i32, ptr %i, align 4
  %inc = add nsw i32 %7, 1
  store i32 %inc, ptr %i, align 4
  br label %for.cond4, !llvm.loop !5

for.end:                                          ; preds = %for.cond4
  br label %for.inc11

for.inc11:                                        ; preds = %for.end
  %8 = load i32, ptr %delta, align 4
  %add = add nsw i32 %8, 399
  store i32 %add, ptr %delta, align 4
  br label %for.cond1, !llvm.loop !7

for.end12:                                        ; preds = %for.cond1
  br label %for.inc13

for.inc13:                                        ; preds = %for.end12
  %9 = load i32, ptr %inc_dec, align 4
  %sub = sub nsw i32 %9, 2
  store i32 %sub, ptr %inc_dec, align 4
  br label %for.cond, !llvm.loop !8

for.end14:                                        ; preds = %for.cond
  store i32 0, ptr %retval, align 4
  br label %return

return:                                           ; preds = %for.end14, %if.then
  %10 = load i32, ptr %retval, align 4
  ret i32 %10
}

declare ptr @brk(ptr noundef) #1

declare ptr @sbrk(i32 noundef) #1

declare i32 @printf(ptr noundef, ...) #1

; Function Attrs: cold
declare void @perror(ptr noundef) #2

attributes #0 = { noinline nounwind optnone ssp uwtable(sync) "frame-pointer"="non-leaf" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+altnzcv,+ccdp,+ccidx,+complxnum,+crc,+dit,+dotprod,+flagm,+fp-armv8,+fp16fml,+fptoint,+fullfp16,+jsconv,+lse,+neon,+pauth,+perfmon,+predres,+ras,+rcpc,+rdm,+sb,+sha2,+sha3,+specrestrict,+ssbs,+v8.1a,+v8.2a,+v8.3a,+v8.4a,+v8a,+zcm,+zcz" }
attributes #1 = { "frame-pointer"="non-leaf" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+altnzcv,+ccdp,+ccidx,+complxnum,+crc,+dit,+dotprod,+flagm,+fp-armv8,+fp16fml,+fptoint,+fullfp16,+jsconv,+lse,+neon,+pauth,+perfmon,+predres,+ras,+rcpc,+rdm,+sb,+sha2,+sha3,+specrestrict,+ssbs,+v8.1a,+v8.2a,+v8.3a,+v8.4a,+v8a,+zcm,+zcz" }
attributes #2 = { cold "frame-pointer"="non-leaf" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+altnzcv,+ccdp,+ccidx,+complxnum,+crc,+dit,+dotprod,+flagm,+fp-armv8,+fp16fml,+fptoint,+fullfp16,+jsconv,+lse,+neon,+pauth,+perfmon,+predres,+ras,+rcpc,+rdm,+sb,+sha2,+sha3,+specrestrict,+ssbs,+v8.1a,+v8.2a,+v8.3a,+v8.4a,+v8a,+zcm,+zcz" }
attributes #3 = { cold }

!llvm.module.flags = !{!0, !1, !2, !3}
!llvm.ident = !{!4}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"uwtable", i32 1}
!3 = !{i32 7, !"frame-pointer", i32 1}
!4 = !{!"Homebrew clang version 19.1.4"}
!5 = distinct !{!5, !6}
!6 = !{!"llvm.loop.mustprogress"}
!7 = distinct !{!7, !6}
!8 = distinct !{!8, !6}
