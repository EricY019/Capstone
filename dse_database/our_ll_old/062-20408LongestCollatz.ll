; ModuleID = './code/062-20408LongestCollatz.c'
source_filename = "./code/062-20408LongestCollatz.c"
target datalayout = "e-m:o-i64:64-i128:128-n32:64-S128-Fn32"
target triple = "arm64-apple-macosx14.0.0"

@.str = private unnamed_addr constant [21 x i8] c"num: %3i length:%3i\0A\00", align 1
@.str.1 = private unnamed_addr constant [13 x i8] c"longest: %i\0A\00", align 1

; Function Attrs: noinline nounwind optnone ssp uwtable(sync)
define i32 @collatzLength(i32 noundef %nIn) #0 {
entry:
  %nIn.addr = alloca i32, align 4
  %n = alloca i64, align 8
  %l = alloca i32, align 4
  store i32 %nIn, ptr %nIn.addr, align 4
  %0 = load i32, ptr %nIn.addr, align 4
  %conv = sext i32 %0 to i64
  store i64 %conv, ptr %n, align 8
  store i32 0, ptr %l, align 4
  br label %while.cond

while.cond:                                       ; preds = %if.end, %entry
  %1 = load i64, ptr %n, align 8
  %cmp = icmp sgt i64 %1, 1
  br i1 %cmp, label %while.body, label %while.end

while.body:                                       ; preds = %while.cond
  %2 = load i64, ptr %n, align 8
  %rem = srem i64 %2, 2
  %cmp2 = icmp eq i64 %rem, 0
  br i1 %cmp2, label %if.then, label %if.else

if.then:                                          ; preds = %while.body
  %3 = load i64, ptr %n, align 8
  %div = sdiv i64 %3, 2
  store i64 %div, ptr %n, align 8
  br label %if.end

if.else:                                          ; preds = %while.body
  %4 = load i64, ptr %n, align 8
  %mul = mul nsw i64 3, %4
  %add = add nsw i64 %mul, 1
  store i64 %add, ptr %n, align 8
  br label %if.end

if.end:                                           ; preds = %if.else, %if.then
  %5 = load i32, ptr %l, align 4
  %inc = add nsw i32 %5, 1
  store i32 %inc, ptr %l, align 4
  br label %while.cond, !llvm.loop !5

while.end:                                        ; preds = %while.cond
  %6 = load i32, ptr %l, align 4
  ret i32 %6
}

; Function Attrs: noinline nounwind optnone ssp uwtable(sync)
define i32 @collatzLengthRec(i32 noundef %n, i32 noundef %l) #0 {
entry:
  %retval = alloca i32, align 4
  %n.addr = alloca i32, align 4
  %l.addr = alloca i32, align 4
  store i32 %n, ptr %n.addr, align 4
  store i32 %l, ptr %l.addr, align 4
  %0 = load i32, ptr %n.addr, align 4
  %cmp = icmp sle i32 %0, 1
  br i1 %cmp, label %if.then, label %if.else

if.then:                                          ; preds = %entry
  %1 = load i32, ptr %l.addr, align 4
  store i32 %1, ptr %retval, align 4
  br label %return

if.else:                                          ; preds = %entry
  %2 = load i32, ptr %n.addr, align 4
  %rem = srem i32 %2, 2
  %cmp1 = icmp eq i32 %rem, 0
  br i1 %cmp1, label %if.then2, label %if.else3

if.then2:                                         ; preds = %if.else
  %3 = load i32, ptr %n.addr, align 4
  %div = sdiv i32 %3, 2
  %4 = load i32, ptr %l.addr, align 4
  %add = add nsw i32 %4, 1
  %call = call i32 @collatzLengthRec(i32 noundef %div, i32 noundef %add)
  store i32 %call, ptr %retval, align 4
  br label %return

if.else3:                                         ; preds = %if.else
  %5 = load i32, ptr %n.addr, align 4
  %mul = mul nsw i32 3, %5
  %add4 = add nsw i32 %mul, 1
  %6 = load i32, ptr %l.addr, align 4
  %add5 = add nsw i32 %6, 1
  %call6 = call i32 @collatzLengthRec(i32 noundef %add4, i32 noundef %add5)
  store i32 %call6, ptr %retval, align 4
  br label %return

return:                                           ; preds = %if.else3, %if.then2, %if.then
  %7 = load i32, ptr %retval, align 4
  ret i32 %7
}

; Function Attrs: noinline nounwind optnone ssp uwtable(sync)
define i32 @main(i32 noundef %argc, ptr noundef %argv) #0 {
entry:
  %retval = alloca i32, align 4
  %argc.addr = alloca i32, align 4
  %argv.addr = alloca ptr, align 8
  %n = alloca i32, align 4
  %maxL = alloca i32, align 4
  %i = alloca i32, align 4
  %curL = alloca i32, align 4
  store i32 0, ptr %retval, align 4
  store i32 %argc, ptr %argc.addr, align 4
  store ptr %argv, ptr %argv.addr, align 8
  store i32 0, ptr %n, align 4
  store i32 0, ptr %maxL, align 4
  store i32 100000, ptr %i, align 4
  br label %for.cond

for.cond:                                         ; preds = %for.inc, %entry
  %0 = load i32, ptr %i, align 4
  %cmp = icmp slt i32 %0, 1000000
  br i1 %cmp, label %for.body, label %for.end

for.body:                                         ; preds = %for.cond
  store i32 0, ptr %curL, align 4
  %1 = load i32, ptr %i, align 4
  %call = call i32 @collatzLength(i32 noundef %1)
  %2 = load i32, ptr %maxL, align 4
  %cmp1 = icmp sgt i32 %call, %2
  br i1 %cmp1, label %if.then, label %if.end

if.then:                                          ; preds = %for.body
  %3 = load i32, ptr %i, align 4
  %4 = load i32, ptr %i, align 4
  %call2 = call i32 @collatzLength(i32 noundef %4)
  %call3 = call i32 (ptr, ...) @printf(ptr noundef @.str, i32 noundef %3, i32 noundef %call2)
  %5 = load i32, ptr %i, align 4
  %call4 = call i32 @collatzLength(i32 noundef %5)
  store i32 %call4, ptr %maxL, align 4
  %6 = load i32, ptr %i, align 4
  store i32 %6, ptr %n, align 4
  br label %if.end

if.end:                                           ; preds = %if.then, %for.body
  br label %for.inc

for.inc:                                          ; preds = %if.end
  %7 = load i32, ptr %i, align 4
  %inc = add nsw i32 %7, 1
  store i32 %inc, ptr %i, align 4
  br label %for.cond, !llvm.loop !7

for.end:                                          ; preds = %for.cond
  %8 = load i32, ptr %n, align 4
  %call5 = call i32 (ptr, ...) @printf(ptr noundef @.str.1, i32 noundef %8)
  %9 = load i32, ptr %retval, align 4
  ret i32 %9
}

declare i32 @printf(ptr noundef, ...) #1

attributes #0 = { noinline nounwind optnone ssp uwtable(sync) "frame-pointer"="non-leaf" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+altnzcv,+ccdp,+ccidx,+complxnum,+crc,+dit,+dotprod,+flagm,+fp-armv8,+fp16fml,+fptoint,+fullfp16,+jsconv,+lse,+neon,+pauth,+perfmon,+predres,+ras,+rcpc,+rdm,+sb,+sha2,+sha3,+specrestrict,+ssbs,+v8.1a,+v8.2a,+v8.3a,+v8.4a,+v8a,+zcm,+zcz" }
attributes #1 = { "frame-pointer"="non-leaf" "no-trapping-math"="true" "stack-protector-buffer-size"="8" "target-cpu"="apple-m1" "target-features"="+aes,+altnzcv,+ccdp,+ccidx,+complxnum,+crc,+dit,+dotprod,+flagm,+fp-armv8,+fp16fml,+fptoint,+fullfp16,+jsconv,+lse,+neon,+pauth,+perfmon,+predres,+ras,+rcpc,+rdm,+sb,+sha2,+sha3,+specrestrict,+ssbs,+v8.1a,+v8.2a,+v8.3a,+v8.4a,+v8a,+zcm,+zcz" }

!llvm.module.flags = !{!0, !1, !2, !3}
!llvm.ident = !{!4}

!0 = !{i32 1, !"wchar_size", i32 4}
!1 = !{i32 8, !"PIC Level", i32 2}
!2 = !{i32 7, !"uwtable", i32 1}
!3 = !{i32 7, !"frame-pointer", i32 1}
!4 = !{!"Homebrew clang version 19.1.4"}
!5 = distinct !{!5, !6}
!6 = !{!"llvm.loop.mustprogress"}
!7 = distinct !{!7, !6}
