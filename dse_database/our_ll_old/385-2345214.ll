...
for.body:                                         ; preds = %for.cond
  ...
  store i64 %1, ptr %nx, align 8
  store i32 1, ptr %t, align 4
  br label %while.cond

while.cond:                                       ; preds = %if.end, %for.body
  %2 = load i64, ptr %nx, align 8
  %cmp1 = icmp ne i64 %2, 1
  br i1 %cmp1, label %while.body, label %while.end

while.body:                                       ; preds = %while.cond
  %3 = load i64, ptr %nx, align 8
  %rem = srem i64 %3, 2
  %cmp2 = icmp eq i64 %rem, 0
  br i1 %cmp2, label %if.then, label %if.else

if.then:                                          ; preds = %while.body
  %4 = load i64, ptr %nx, align 8
  %div = sdiv i64 %4, 2
  store i64 %div, ptr %nx, align 8
  br label %if.end

if.else:                                          ; preds = %while.body
  %5 = load i64, ptr %nx, align 8
  %mul = mul nsw i64 3, %5
  %add = add nsw i64 %mul, 1
  store i64 %add, ptr %nx, align 8
  br label %if.end
...
for.end:                                          ; preds = %for.cond
  %12 = load i64, ptr %mn, align 8
  %call = call i32 (ptr, ...) @printf(ptr noundef @.str, i64 noundef %12)
  ret i32 0
}
