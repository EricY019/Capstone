...
from utils import create_dir_if_not_exists, get_root_path, natural_keys, get_save_path
# get_save_path = "/local2/shared_data/hugging_face_c/graphs_harp"
...
'''
type_graph = 'test' 
BENCHMARK = 'onesampletest'
target = ['onesampletest']
'''
# TODO: all graphs will be saved to public local2 folder 
processed_gexf_folder = join(get_save_path(), f'{type_graph}/{BENCHMARK}/processed')
auxiliary_node_gexf_folder = join(get_save_path(), f'{type_graph}/{BENCHMARK}/processed/extended-pseudo-block-base/')

all_files_folder = join(get_save_path(), f'{type_graph}/{BENCHMARK}')

#FIXME: NO processed result should be included
def file_name_extraction(directory):
    ...
    for root, _, files in os.walk(directory):
        for file in files:
            ...
            if not filename_without_ext.endswith("_processed_result"):
                            unique_filenames.add(filename_without_ext)

    # Convert the set to a sorted list (optional)
    filename_array = sorted(list(unique_filenames))
    print("Extracted file lengths: ", len(filename_array))
    return filename_array
print("Extracting file names from: ", all_files_folder)
KERNELS = file_name_extraction(all_files_folder)

#print("kernel names: ", KERNELS)
# ['000-1178_select-sort', '000-138_missing_positive', '000-1198_heapsort', '000-11974_exercise2-4']#[]
ALL_KERNEL = {BENCHMARK: KERNELS}
...
def graph_generator(name, path, benchmark, generate_programl = False, csv_dict=None):
    ...
    if generate_programl: #TODO: modify the script's position
        p = Popen(f"{get_root_path()}/clang_script.sh {name} {path} {type_graph}", shell = True, stdout = PIPE)
        p.wait()
        # only generates llvm file (already generated)
...
def get_for_blocks_info(name, path):
    with open(join(path, f'{name}.ll'), 'r') as f_llvm:# name,  TODO: path modification
        lines_llvm = f_llvm.readlines()
...
def write_csv_file(csv_dict, csv_header, file_path):
    with open(join(get_save_path(), file_path), mode = 'w') as f:
...
def run_graph_gen(mode='initial', connected=True, target=target, ALL_KERNEL=ALL_KERNEL):
    test = 'original'
    ...
    if mode == 'initial':
        for BENCHMARK in target:
            processed_gexf_folder = join(get_save_path(), f'{type_graph}/{BENCHMARK}/processed/')# Address to save the processed gexf files
            create_dir_if_not_exists(processed_gexf_folder)
            for kernel in ALL_KERNEL[BENCHMARK]:
                try:
                    print('####################')
                    print(mode, ' Now processing:', kernel)
                    ## modify the path below
                    path = join(get_save_path(), f'{type_graph}/{BENCHMARK}/')#, f'{kernel}'TODO
                    if not exists(path):
                        create_dir_if_not_exists(path)
                        ## this file needs to be run in programl mode first for this mode to work
                        kernel_dir = join(get_save_path(), f'{BENCHMARK}/sources/**')#/* TODO:  c file path
                        kernel_path = [f for f in iglob(kernel_dir, recursive=True) if f'{kernel}' in f]
                    ...
                    print(f"An error occurred while processing {kernel}: {e}")
                finally:    
                    print()
        ...
        for BENCHMARK in target:
            processed_gexf_folder = join(get_save_path(), f'{type_graph}/{BENCHMARK}/processed')
            if connected:
                auxiliary_node_gexf_folder = join(get_save_path(), f'{type_graph}/{BENCHMARK}/processed/extended-pseudo-block-connected/')
            else:
                auxiliary_node_gexf_folder = join(get_save_path(), f'{type_graph}/{BENCHMARK}/processed/extended-pseudo-block-base/')
    ...
    elif mode == 'hierarchy':#ADD except for the hierarchy
        for BENCHMARK in target:
            llvm_base_folder = join(get_save_path(), f'{type_graph}/{BENCHMARK}/')
            auxiliary_node_gexf_folder = join(get_save_path(), f'{type_graph}/{BENCHMARK}/processed/extended-pseudo-block-connected/')
            dest_path = join(get_save_path(), f'{type_graph}/{BENCHMARK}/processed/extended-pseudo-block-connected-hierarchy/')
            ...
            for kernel in ALL_KERNEL[BENCHMARK]:
                try:
                    ...
                    print('Processing completed successfully for:', kernel)
                except Exception as e:
                    print(f"An error occurred while processing {kernel}: {e}")
                finally:
                    print()
...
if __name__ == '__main__':
    ...
    # pass
# cd /home/jade1st/pj/Pretrain
# python -W ignore graph_gen_pt.py > graph_gen_pt_23k_following_parts.log 2>&1