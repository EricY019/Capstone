...
def model_update(model, losses_list, loss, epoch, plot_test, tag):
    ...
    if losses_list and loss < min(losses_list):
        if FLAGS.save_model:
            ...
            # torch.save(model.state_dict(), join(saver.model_logdir, f"{tag}_model_state_dict_e{epoch}_l_{loss}.pth"))
        plot_test = True
    if FLAGS.save_model:
        if epoch % FLAGS.save_freq == 0:
            os.makedirs(join(saver.model_logdir, 'ckpts'), exist_ok=True)
            path = join(saver.model_logdir, 'ckpts', f"{tag}_model_state_dict_e{epoch}_l{loss}.pth")
...
def train_main(dataset, pragma_dim = None, val_ratio=FLAGS.val_ratio, test_ratio=FLAGS.val_ratio, resample=-1):
    ...
    if FLAGS.model_path != None:
        ...
        # model.load_state_dict(torch.load(model_path, map_location=torch.device('cpu')))
        # only load the parameter that's exist in both the model_path and model
        # model.load_state_dict(torch.load(model_path, map_location=torch.device('cpu')), strict=False)
        checkpoint_state_dict = torch.load(model_path, map_location=torch.device('cpu'))
        model_state_dict = model.state_dict()
        common_keys = set(checkpoint_state_dict.keys()).intersection(model_state_dict.keys())
        filtered_checkpoint_state_dict = {k: checkpoint_state_dict[k] for k in common_keys}
    ...
    for epoch in range(FLAGS.epoch_num):
        ...
        if len(val_loader) > 0 and epoch % FLAGS.val_freq == 0:
        ...
        if len(test_loader) > 0 and epoch % FLAGS.test_freq == 0:
        ...
        if len(val_loader) > 0 and len(test_loader) > 0 and epoch % FLAGS.val_freq == 0 and epoch % FLAGS.test_freq == 0:
        ...
        elif len(test_loader) > 0 and epoch % FLAGS.test_freq == 0:
            log_loss(loss_dict_test, gae_loss_test, "Test")
