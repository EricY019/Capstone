...
class MyOwnDataset(Dataset):
    ...
    def get(self, idx):
#         466 ['234-27088bench']
# 1404it [00:29, 46.28it/s]1405 ['304-13860simple']
# 1444it [00:30, 44.64it/s]1448 ['274-14823matrix-row-column-order']
# 1484it [00:31, 47.28it/s]1485 ['118-22819sum']
        dead = [223, 466, 1405, 1448, 1485 ]
        
        # outlier = [7, 14, 23, 79, 96, 100, 114, 132, 164, 175, 188, 194, 208, 222, 237, 240, 254, 257, 261, 269, 296, 301, 313, 336, 411, 420, 425, 426, 432, 463, 472, 536, 541, 563, 570, 592, 606, 609, 660, 666, 693, 695, 708, 713, 749, 753, 756, 776, 793, 809, 863, 870, 889, 900, 912, 919, 923, 924, 926, 955, 965, 980, 994, 995, 1006, 1007, 1032, 1038, 1040, 1055, 1103, 1111, 1114, 1131, 1134, 1137, 1140, 1154, 1166, 1183, 1191, 1195, 1272, 1289, 1307, 1309, 1310, 1315, 1333, 1342, 1352, 1401, 1403, 1416, 1439, 1467, 1469, 1470, 1473, 1487, 1497, 1527]
        
        outlier = [[7, 14, 23, 56, 60, 67, 79, 96, 100, 112, 114, 132, 145, 164, 166, 175, 188, 194, 208, 222, 227, 237, 238, 240, 248, 254, 257, 261, 269, 279, 284, 296, 301, 303, 313, 328, 336, 352, 365, 379, 397, 411, 420, 424, 425, 426, 432, 463, 472, 494, 536, 541, 553, 555, 563, 570, 574, 592, 606, 609, 626, 660, 666, 667, 693, 695, 701, 708, 713, 734, 749, 753, 756, 776, 793, 809, 814, 816, 839, 840, 859, 862, 863, 870, 889, 896, 900, 912, 918, 919, 923, 924, 926, 955, 965, 980, 986, 994, 995, 1006, 1007, 1015, 1020, 1032, 1038, 1040, 1055, 1103, 1111, 1114, 1131, 1134, 1137, 1140, 1154, 1166, 1183, 1191, 1195, 1200, 1254, 1271, 1272, 1285, 1289, 1307, 1309, 1310, 1315, 1317, 1333, 1342, 1352, 1353, 1370, 1388, 1390, 1401, 1403, 1405, 1416, 1439, 1449, 1452, 1467, 1469, 1470, 1473, 1476, 1487, 1497, 1527, 1535, 1557]]
        
        # outlier += [7, 14, 23, 1166]
        outlier += [7, 14, 23, 126, 155, 353, 496, 540, 631, 652, 670, 759, 785, 826, 1017, 1030, 1085, 1118, 1166, 1171, 1567]
        
        while idx in dead or idx in outlier:
            # random between 0-1574
            idx = np.random.randint(0, 1574)
        if hasattr(self, 'data_files'):
            fn = self.data_files[idx]
        else:
            fn = osp.join(SAVE_DIR, 'data_{}.pt'.format(idx))
...
def get_data_list():
    ...
    for gexf_file in tqdm(GEXF_FILES[0:]): 
        ...
        # saver.log_info(f'gname: {gname}')
        # saver.log_info(f"n: {n}")
        all_gs[gname] = g
